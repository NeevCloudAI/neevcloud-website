...
const StatePersistencePanel = () => {
  return (
    <div className="max-w-2xl rounded-xl bg-white p-7.5 shadow-sm">
      <div className="flex items-center justify-between border-b border-gray-01 pb-4">
        <Text as="h5" className="font-semibold uppercase">
          December usage
        </Text>
        <Text textColor="gray-03">Billing period · May 1 to May 31</Text>
      </div>
      ...
      <div className="flex flex-col">
        {BILLING_ROWS.map((row) => (
          <div
            key={row.label}
            className="flex items-center justify-between py-3.5 border-b border-gray-01"
          >
            <Text as="h6" className="font-semibold">
              {row.label}
            </Text>
            <Text className="text-sm font-medium text-gray-05">
              {row.value}
            </Text>
          </div>
        ))}
      ...
      <div className="mt-4 md:mt-6 rounded-lg bg-primary/10 px-4 py-3 text-center border border-primary-90">
        <Text
          as="small"
