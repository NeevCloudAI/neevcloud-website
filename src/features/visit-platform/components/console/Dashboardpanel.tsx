...
/**
 * Right-panel mockup for the "Dashboard Overview" tab.
 * Shows stat cards (GPU UTIL, REQUESTS, SPEND), a wave chart, and tag pills.
 */
const DashboardPanel = () => {
  return (
    <ConsoleMockupCard title="DASHBOARD">
      <div className="border-b border-gray-60 mb-5 mt-2.5"></div>
      {/* Stat cards */}
      <div className="grid grid-cols-3 gap-4 mb-4 px-7.5 ">
        {DASHBOARD_STATS.map((stat) => (
          <div
            key={stat.label}
            className="flex flex-col gap-1 rounded-lg bg-black p-4"
          >
            <Text
              as="h6"
            ...
            <Text as="h2" className="font-semibold" textColor="white">
              {stat.value}
            </Text>
      ...
      <div className="border-b border-gray-60 mb-5 mt-3.5"></div>

      {/* Tags */}
      <div className="mt-5 pb-7.5 flex flex-wrap gap-1.5 px-7.5">
        {DASHBOARD_TAGS.map((tag) => (
          <Text
            as="small"
            key={tag}
            className="rounded-full bg-primary px-2.5 py-1 font-medium"
            textColor="white"
          >
            {tag}
          </Text>
