...
const AiInferenceScenarioCard = ({ item }: AiInferenceScenarioCardProps) => {
  ...
  return (
    <article className="flex h-full flex-col rounded-md bg-white p-5">
      <div className="w-fit rounded-md bg-primary-25 p-3.5">
        <Icon className="size-[22px] text-primary-70" aria-hidden="true" />
      ...
      <ul className="mt-5 flex list-none flex-wrap gap-1 p-0">
        {item.tags.map((tag) => (
          <li
            key={tag}
            className="rounded-md bg-primary-170 px-2 py-1 font-mono text-xs text-black-5"
          >
            {tag}
          </li>
        ))}
